...
def main():
  ...
  for k in range(m):
    ...
    z[0,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=0.001))
    z[1,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=0.01))
    z[2,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=0.1))
    z[3,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=1.0))
    z[4,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=2.0))
    z[5,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=10.0))
    z[6,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=50.0))
    z[7,k] = run(x_train, y_train, x_test, y_test, SVC(kernel="linear", C=100.0))

  prettyPrint(z, 0, "SVC(kernel=linear, C=0.001)")
  prettyPrint(z, 1, "SVC(kernel=linear, C=0.01)")
  prettyPrint(z, 2, "SVC(kernel=linear, C=0.1)")
  prettyPrint(z, 3, "SVC(kernel=linear, C=1.0)")
  prettyPrint(z, 4, "SVC(kernel=linear, C=2.0)")
  prettyPrint(z, 5, "SVC(kernel=linear, C=10.0)")
  prettyPrint(z, 6, "SVC(kernel=linear, C=50.0)")
  prettyPrint(z, 7, "SVC(kernel=linear, C=100.0)")
main()
